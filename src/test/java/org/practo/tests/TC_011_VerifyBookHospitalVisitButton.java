...
public class TC_011_VerifyBookHospitalVisitButton extends BaseTest {
    ...
    private static final Logger logger =
            LogManager.getLogger(
                    TC_011_VerifyBookHospitalVisitButton.class);
    ...
    public void verifyBookHospitalVisitButtonIsFunctional(
            Map<String, String> data) {

        logger.info(
                "Starting TC_011 - Verify Book Hospital Visit Button");
        ...
        logger.info(
                "Book Hospital Visit button is displayed");
        ...
        logger.info(
                "Book Hospital Visit button is enabled");

        String previousPageTitle =
                commonCode.getPageTitle();
        ...
        logger.info(
                "Clicking Book Hospital Visit button");

        bookHospitalButton.click();

        boolean newWindowOpened =
                commonCode.switchToNewWindowIfAvailable(
                        parentWindow);

        if (newWindowOpened) {

            logger.info(
                    "New window opened and switched successfully.");

        } else {

            logger.info(
                    "No new window opened. Continuing in the same window.");

            commonCode.waitForUrlToBeChanged(
                    previousPageUrl);
        }
        ...
        logger.info(
                "TC_011 Passed: Book Hospital Visit button is functional.");
    }
}