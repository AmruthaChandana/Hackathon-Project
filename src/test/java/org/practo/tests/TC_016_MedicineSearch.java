...
public class TC_016_MedicineSearch extends BaseTest {
    ...
    public void verifyMedicineSearch() {
        ...
        logger.info(
                "Starting TC_016 - Medicine Search");
        ...
        Assert.assertTrue(
                medicinesPage.isMedicinePresentInResults(
                        medicineName),
                "Medicine is not present in search results: "
                        + medicineName
        );

        String matchedMedicine =
                medicinesPage.getMatchedMedicineName(
                        medicineName);
        ...
        logger.info(
                "Printing first five medicines");

        medicinesPage.printFirstFiveMedicines();

        logger.info(
                "TC_016 Passed: Medicine search completed successfully");
    }
}