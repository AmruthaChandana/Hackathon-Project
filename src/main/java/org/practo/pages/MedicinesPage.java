...
public class MedicinesPage {
    ...
    @FindBy(xpath = "//div[contains(@class,'image-carousel--product_add-cart')]")
    private WebElement firstAddToCartButton;

    @FindBy(xpath = "//a[@href='/order/cart']")
    ...
    private <T> T waitUntil(
            java.util.function.Function<WebDriver, T> condition) {
        ...
        try {
            ...
            for (WebElement suggestion : medicineSuggestions) {
                try {
                    ...
                    if (!text.isEmpty() && text.toLowerCase()
                            .contains(expectedMedicineName.toLowerCase())) {
                        return true;
                    }
                } catch (Exception e) {
