...
public class HospitalPage {
    ...
    private static final Logger logger =
            LogManager.getLogger(HospitalPage.class);
    ...
    public String cleanAddressText(
            String addressText) {
    ...
    public WebElement getHospitalCardFromHospitalName(
    ...
    public boolean isHospitalOpen24x7(
            WebElement hospitalCard) {

        String cardText =
                hospitalCard.getText();

        return cardText != null
                && cardText.contains("Open 24x7");
    }

    public boolean hasHospitalRating(
            WebElement hospitalCard) {

        return !getHospitalRatingText(
                hospitalCard)
                .equals("RATING_NOT_AVAILABLE");
    }

    public String getHospitalRatingText(
            WebElement hospitalCard) {
        ...
        try {
            ...
            if (ratingText == null) {
                ...
                logger.warn(
                        "Rating not available for hospital card.");
            ...
            logger.error(
                    "Exception while fetching hospital rating.",
                    e);
    ...
    public double getHospitalRatingValue(
            WebElement hospitalCard) {

        try {

            String ratingText =
                    getHospitalRatingText(
                            hospitalCard);
            ...
            logger.warn(
                    "Unable to convert hospital rating to numeric value."
            );
