...
public class TC_010_VerifyFirstNHospitals extends BaseTest {
    private static final Logger logger = LogManager.getLogger(TC_010_VerifyFirstNHospitals.class);
    ...
    public void verifyFirstNHospitalsAreCheckedAndMatchingHospitalsAreAddedToList(Map<String, String> data) {
        logger.info("Starting TC_010 - Verify First N Hospitals");
        ...
        logger.info("Required Match Count: {}", requiredMatchCount);
        logger.info("Minimum Rating: {}", minimumRating);
        ...
        while (matchingHospitals.size() < requiredMatchCount) {
            ...
            for (int i = checkedCount; i < hospitalNames.size(); i++) {
                ...
                try {
                    ...
                    logger.warn("Skipped one hospital because Open 24x7 or rating details were missing.");
                }
            }
            ...
            if (!moreResultsLoaded) {
                logger.info("No more hospital results available.");
                break;
            }
        }

        // Step 3: Print Matching Hospitals
        logger.info("Total hospitals checked: {}", checkedCount);
        logger.info("Matching hospitals stored in list: {}", matchingHospitals.size());
        logger.info("Matching Hospital Details:");
        ...
        logger.info("TC_010 Passed: Hospitals were checked and matching hospitals were added to list.");
    }
}